...
| Spec target | Wasm 2.0 baseline: multi-value, bulk memory, reference types, sign-extension, nontrapping conversions, fixed-width SIMD — plus multiple memories, extended constant expressions, tail calls, and typed function references from wasm 3.0 |
...
| Typed func refs | The funcType handle owns its reference types: `sig.ref` (non-null) / `sig.refNull`. Signatures INTERN per module (identical shapes → one handle; `fn.type` is lazy), so handle identity is signature identity. `fn.ref()` is precisely typed; upcasts (`ref`→`refNull`→`funcref`) are value-exact promotions — no subtyping lattice, no downcasts (those are GC-only). `sig.ref.of(x)` is the trapping nullable→non-null bridge (bool.of precedent, NOT cast — it's not free). `sig.call(ref, …)` → call_ref; return position tail-calls. Tables take `sig.refNull` (non-null tables have no default). Non-null locals/temps are lowered to nullable slots + ref.as_non_null on read — wasm's structural definite-assignment rules don't fit relooped output; params/globals/results keep true non-null types. Deferred: br_on_null/br_on_non_null |
